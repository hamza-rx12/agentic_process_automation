...
from app.configs.environment_vars.browser_settings import browser_settings
import os
...
class AppConfig:
    ...
    def create_external_mcp_servers() -> dict:
        """Build the Playwright stdio MCP server."""
        from claude_agent_sdk.types import McpStdioServerConfig

        b = browser_settings
        try:
            w, h = b.VIEWPORT.split("x")
        except ValueError:
            w, h = "1280", "720"
    ...
    def get_allowed_tools() -> list[str] | None:
        """Browser agent only uses Playwright MCP tools."""
